...
# Exclude the influential samples
df_sens_infl <- df_low %>%
  filter(!Study %in% influential_samples)
...
sens_infl_summary <- data.frame(
  ...
  samples_included = glmm_sens_infl$k
)

knitr::kable(sens_infl_summary, caption = "Sensitivity Analysis: Low Trajectory (Excluding Influential samples)")
...
df_noinf <- df_moderate %>% filter(!Study %in% influential_samples)
...
sens_infl_summary <- data.frame(
  ...
  samples_included = glmm_noinf$k
)

knitr::kable(sens_infl_summary, caption = "Sensitivity Analysis: Moderate Trajectory (Excluding Influential samples)")
```
