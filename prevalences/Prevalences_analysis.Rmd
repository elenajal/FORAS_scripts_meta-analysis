...
overview <- tibble(
  `Author(s), Year_SampleName` = df_moderation$Study,
  ...
  arrange(`Author(s), Year_SampleName`)
...
if (knitr::is_html_output() || knitr::is_latex_output()) {
  ...
  overview %>%
    ...
    knitr::kable(format = fmt, caption = "General Descriptive Variables per Sample", booktabs = TRUE, linesep = "") |>
    kableExtra::kable_styling(full_width = FALSE, position = "left",
                              bootstrap_options = c("striped", "hover", "condensed")) |>
    kableExtra::column_spec(1, bold = TRUE) |>
    kableExtra::row_spec(0,  extra_css = "font-family: 'Times New Roman', Times, serif;") |>
    kableExtra::row_spec(1:nrow(overview), extra_css = "font-family: 'Times New Roman', Times, serif;") |>
...
## Descriptives Table Export

```{r big-descriptives-table}
# Export to word
overview_fmt <- overview %>%
  mutate(
    `Sample Size`       = format(`Sample Size`, big.mark = ",", trim = TRUE),
    `Mean Age`          = ifelse(is.na(`Mean Age`), NA, sprintf("%.1f", `Mean Age`)),
    `Percentage Women`  = ifelse(is.na(`Percentage Women`), NA, sprintf("%.1f%%", `Percentage Women`))
...
ft <- flextable::flextable(overview_fmt) |> 
  flextable::align(j = c("Sample Size","Mean Age","Percentage Women","Number of TP assessments","Number of Trajectories Found"), align = "right", part = "all") |> 
  flextable::align(j = c("Author(s), Year_SampleName","Assessed Trauma Type"), align = "left", part = "all") |> 
  flextable::bold(part = "header", bold = TRUE) |> flextable::fontsize(part = "all", size = 8) |> 
  flextable::font(part = "all", fontname = "Times New Roman") |> 
  flextable::border_outer(part = "all", border = officer::fp_border(color = "black", width = 1)) |> 
  flextable::border_inner_h(border = officer::fp_border(color = "transparent", width = 0), part = "all") |> 
  flextable::border_inner_v(border = officer::fp_border(color = "transparent", width = 0), part = "all") |> 
  flextable::border(i = 1, border.bottom = officer::fp_border(color = "black", width = 1), part = "header") |> 
  flextable::autofit()

# docx export
flextable::save_as_docx("Table 1" = ft, path = "Descriptives_Table.docx", pr_section = officer::prop_section(
...
.labels_from_study <- function(fit) { # Gets the model’s data via .get_model_data(fit) and uses .used_rows() to take only rows actually used in the fit.
