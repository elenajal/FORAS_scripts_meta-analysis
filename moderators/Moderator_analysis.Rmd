...
# correct labeling error
df_moderation$Health_First[df_moderation$MID == "M1356_2"] <- "No"

# add level "HP/first responder" to dataset
df_moderation <- df_moderation %>%
  mutate(Trauma_type = case_when(
    Health_First == "Yes" ~ "HP/first responder",
    TRUE ~ as.character(Trauma_type)
  ))
...
Civilian vs Military (Levels: Civilian, Military); Discrete vs non-discrete trauma (Levels: Non-discrete, Discrete); Occupational trauma (Levels: No, Yes); Trauma exposure (Levels: Interpersonal trauma, Non-interpersonal trauma); Trauma type (Levels: Combat, Injury, Natural disaster, HP/first responder, Other)
...
trauma_vars <- c(
  "Discrete", "Trauma_type", "Military", "Occupational_trauma", "Trauma_exposure"
  )
...
MOD_SPECS <- list(
  ...
  list(var="Location",              type="cat", label="Country income",
       keep=c("High", "ML"), ref="ML",
       levels=c(High="High income countries",
                ML="Mid-, low- income")),
  ...
  list(
    ...
    type="cat",
    label="Trauma type",
    keep=c("Combat","Natural","Injury", "HP/first responder"), 
    ref="Injury",
    levels=c(
      Combat      = "Combat",
      Injury      = "Injury",
      ...
      "HP/first responder" = "HP or First Responder"
    ),
    na_level = "Other"
  ),
...
LEVEL_MAP <- purrr::map_dfr(MOD_SPECS, \(spec) {
  ...
  # Initialize the level data frame
  ...
  # 2. Handle na_level (missing values)
  if (!is.null(spec$na_level)) {
    # map NA (the original missing value) to the pretty na_level.
    df_na <- tibble::tibble(
      moderator = spec$var,
      orig      = NA_character_, 
...
make_combined_mod_table <- function(df_mod,
  ...
  trajectory_cols <- c("Low","Decreasing","Increasing","High","Moderate")
  ...
  # 1. Clean LEVEL_MAP
  LEVEL_MAP <- LEVEL_MAP %>%
    mutate(orig = trimws(orig)) %>%
    distinct(moderator, pretty, .keep_all = TRUE)

  # 2. assign group (Background / Trauma / Method / Other)
  ...
  recode_categorical <- function(x_raw, spec, spec_map) {
    ...
    # restrict to 'keep' values if specified
    ...
    non_na_map <- spec_map %>% filter(!is.na(orig))
    na_map     <- spec_map %>% filter(is.na(orig))

    if (nrow(non_na_map) > 0) {
      recode_map <- setNames(non_na_map$pretty, non_na_map$orig)
      x <- dplyr::recode(x, !!!recode_map, .default = NA_character_)
    } else {
      x <- as.character(x)
    }

    if (nrow(na_map) >= 1) {
      x[is.na(x)] <- na_map$pretty[1]
    }
  ...
  # 4. OVERALL SUMMARIES
  overall_rows <- map_dfr(MOD_SPECS, function(spec) {
    ...
    if (type == "cont") {
      ...
      Mean <- mean(x_raw, na.rm = TRUE)
      SD   <- sd(x_raw, na.rm = TRUE)
      ...
      tibble(
        ...
        Type           = "Continuous",
        Level          = "",
        OverallSummary = glue("{round(Mean, 2)} (SD = {round(SD, 2)}), {round(Min, 2)}–{round(Max, 2)}")
      )

    } else {

      spec_map <- LEVEL_MAP %>% filter(moderator == var)

      # recode to pretty labels
      x_rec <- recode_categorical(x_raw, spec, spec_map)

      # full list of level labels (pretty)
      all_levels <- tibble(Level = spec_map$pretty)

      tab_df <- tibble(Level = x_rec) %>%
        filter(!is.na(Level)) %>%
        count(Level, name = "N") %>%
        mutate(P = N / sum(N))

      full_df <- all_levels %>%
        left_join(tab_df, by = "Level") %>%
        mutate(
          N = replace_na(N, 0L),
          P = replace_na(P, 0),
  ...
  traj_rows <- map_dfr(MOD_SPECS, function(spec) {
    ...
    if (type == "cont") {
      ...
      mean_vals <- sapply(trajectory_cols, function(traj) {
        vals <- df_mod[[var]][df_mod[[traj]] == 1]
        mean(vals, na.rm = TRUE)
      })

      sd_vals <- sapply(trajectory_cols, function(traj) {
        vals <- df_mod[[var]][df_mod[[traj]] == 1]
        sd(vals, na.rm = TRUE)
      })
      ...
      map_dfr(spec_map$pretty, function(lev_pretty) {
        ...
        row_vals <- sapply(trajectory_cols, function(traj) {
          ...
          if (denominator == 0) {
            ...
            perc <- 100 * numerator / denominator
            sprintf("%.1f%%", perc)
          }
        })
  ...
  final_df <- overall_rows %>%
    left_join(traj_rows, by = c("Moderator", "Level"))

  # order groups and moderators according to MOD_SPECS
  group_levels <- c(
    "Background characteristics",
    "Trauma characteristics",
    "Methodological characteristics",
  ...
  mod_order <- map_chr(MOD_SPECS, "label")
  ...
  # 7. Render GT table
  gt(final_df, groupname_col = "Group") %>%
    cols_label(
      Moderator      = md("**Moderator**"),
      Type           = md("**Type**"),
      Level          = md("**Level**"),
      OverallSummary = md("**Overall Summary**"),
      Low            = md("**Low**"),
      Decreasing     = md("**Decreasing**"),
      Increasing     = md("**Increasing**"),
      High           = md("**High**"),
      Moderate       = md("**Moderate**")
    ) %>%
    cols_align(
      align = "left",
      columns = c(Moderator, Type, Level, OverallSummary)
    ) %>%
    cols_align(
      align = "right",
    ...
    tab_spanner(
      label   = "Trajectory distributions (% within trajectory)",
      columns = trajectory_cols
    ) %>%
    tab_header(
      title = md("**Moderator Summary with Trajectory Distributions**"),
    ...
    opt_table_lines() %>%
    # Bold the first occurrence of each moderator
    tab_style(
      style = cell_text(weight = "bold"),
      locations = cells_body(
        columns = Moderator,
        rows = !duplicated(Moderator)
      )
    ) %>%
    # Blank repeated moderator names for cleaner look
    text_transform(
      locations = cells_body(columns = Moderator),
      fn = function(x) {
        x[duplicated(x)] <- ""
        x
      }
...
make_group_table <- function(group_res, title, na_label = .na_label) {
  ...
  gt::gt(out) %>%
    ...
    gt::cols_label(
      ...
      Delta_Marginal_Cont = gt::md("Marginal Δ"), 
...
Differences (Marginal Δ) are significant at α = .05. 
...
generate_correlation_gt <- function(df, 
  ...
  if (show_warnings) {
    ...
    if (length(exclude_vars) > 0) {
      ...
      dropped_pretty <- MOD_LABELS[exclude_vars]
    
      cat("<hr>")
      cat("<br><strong>Warning: Highly Correlated Variables Removed</strong><br>")
      cat(
        paste0(
          "The following continuous predictors were dropped due to high correlation ",
          "(|r| > 0.90) based on their weaker association with the outcome ",
          "for the <em>", trajectory, "</em> trajectory:<br><br>",
          "<em>",
          paste(dropped_pretty, collapse = ", "),
          "</em><br><br>"
        )
      )
...
Further, to account for data sparcity and high overlap, some categorical variables will be combined/omitted from the analysis. Due to high overlap between the variable "Military" and the level "Combat" of the variable "Trauma type", the variable "Military" will also be excluded from all analyses for this Bayesian model.
...
apply_mod_specs <- function(df, mod_specs) {
  ...
  for (spec in mod_specs) {
    ...
    if (identical(type, "cat")) {
      ...
      # Handle missing values (NA) by mapping them to the na_level string
      if (!is.null(spec$na_level)) {
...
prepare_data_brms <- function(df, mod_specs = MOD_SPECS, exclude = NULL) {
  ...
  # Apply all variable specifications (recode, filter, set ref level)
    ...
    df <- df %>%
      dplyr::mutate(
        dplyr::across(
          ...
          ~ as.numeric(scale(suppressWarnings(as.numeric(.))))
        )
      )
  }

  #  Convert binary factors to 0/1 numeric
  ...
  # Ensure all other categorical variables are factors
  ...
  # Drop unused levels only for the remaining factors (non-binary)
  df <- droplevels(df)
...
get_binary_cat_vars <- function(df, cat_vars_all) {
  ...
  for (v in intersect(cat_vars_all, names(df))) {
    x_non_na <- df[[v]][!is.na(df[[v]])]
    if (is.factor(x_non_na) && nlevels(x_non_na) == 2) {
...
factor_to_01 <- function(fct_var) {
  ...
  # Converts factor levels (1, 2) to (0, 1), where the reference level (1) becomes 0.
...
fit_brms_models_mi <- function(df,
  ...
  # Use the input data frame 'df' directly
  ...
  # 1. Identify all variables that need imputation
  vars_with_na_raw <- model_vars[
    sapply(df[, model_vars, drop = FALSE], function(x) any(is.na(x)))
  ]
  
  # 2. Split missing variables by imputed type (Numeric vs. Factor)
  # Numeric includes original continuous and the new 0/1 binary variables (Gaussian family)
  vars_numeric_na <- vars_with_na_raw[
    sapply(df[, vars_with_na_raw, drop = FALSE], is.numeric)
  ]
  
  # Factor variables (Multi-level categorical, also imputed with Gaussian approximation)
  vars_factor_na <- vars_with_na_raw[
    sapply(df[, vars_with_na_raw, drop = FALSE], is.factor)
  ]
  
  vars_with_na <- c(vars_numeric_na, vars_factor_na)
  
  # Predictors without NAs (no imputation needed)
  vars_without_na <- setdiff(model_vars, vars_with_na)
  
  # Build RHS terms for the main model (Outcome ~ Predictors)
  rhs_terms <- c(
    if (length(vars_with_na)) paste0("mi(", vars_with_na, ")"),
    setdiff(vars_without_na, "Study") 
  )
  
  # Main formula for the outcome (binomial GLMM)
  main_formula <- bf(
    stats::as.formula(
      paste0(
        success_col_name,
  ...
  # Case 2: MI for Numeric (Continuous & 0/1) and Factor (Multi-level) vars
  
  # Imputation formulas for Numeric variables (Gaussian family)
  mi_formulas_numeric <- lapply(vars_numeric_na, function(v) {
    brms::bf(stats::as.formula(paste0(v, " | mi() ~ 1")), family = stats::gaussian()) 
  })
  
  # Imputation formulas for Factor variables (Gaussian approximation for simplicity/compatibility)
  mi_formulas_factor <- lapply(vars_factor_na, function(v) {
    message(paste("Warning: Imputing multi-level factor", v, "using Gaussian approximation."))
    brms::bf(stats::as.formula(paste0(v, " | mi() ~ 1")), family = stats::gaussian()) 
  })
  
  mi_formulas <- c(mi_formulas_numeric, mi_formulas_factor)
...
fit_brms_trajectory_models <- function(df,
  ...
  # Filtering variables based on exclusion list
...
Finally, the multivariate models are compared using leave-one-out cross-validation (LOO-CV). This provides an estimate of the relative predictive accuracy of the three model specifications and helps assess whether additional moderator domains improve the overall model fit for each trajectory. LOO-CV ranks models by how well they predict unseen data using ELPD, where higher ELPD values indicate better predictive accuracy. The best model has ΔELPD = 0, and other models have more negative ΔELPD values the worse they perform, with the SE showing how confidently we can distinguish those differences. Model weights summarize each model’s relative predictive support, where higher weights signal a model that is more likely to generalize well to new data.
 ...
 # Remove 'mi' (multiple imputation indicator)
    ...
    dplyr::mutate(
      ...
      Predictor = ifelse( # Create the final predictor label
        Level == "" | is.na(Level), # If no level suffix, use the base label (e.g., for continuous variables)
        VarLabel,
        # If a level exists, combine base label and level in parentheses
  ...
  effect_labels <- stats::setNames( # Create labels for Effect columns
    gsub("Effect_", "Effect [95% CI] – ", effect_cols),
    effect_cols
  )
  
  importance_labels <- stats::setNames( # Create labels for Important columns
    gsub("Important_", "Important? – ", importance_cols),
    importance_cols
  )
  ...
  coefficient_wide %>%
    ...
    gt::data_color( # Color cells based on 'Important' status
      ...
      fn  = function(x) dplyr::case_when(
        ...
        x == "No"  ~ "lightgray",   # Light gray for 'No'
        TRUE       ~ "white"
      )
    ) %>%
    gt::tab_options(
...
diagnostic_gt_brms <- function(model,
  ...
  # Canonicalize coefficient names
  canonical_term <- function(term) {
    term <- gsub("`", "", term)
    term <- sub("^[^_]+_", "", term)   # drop outcome prefix
    term <- sub("^mi", "", term)       # drop mi prefix
    term
  }

  # Detect categorical moderator (longest name wins)
  cat_vars <- names(CAT_SPECS)
  cat_vars <- cat_vars[order(nchar(cat_vars), decreasing = TRUE)]

  detect_cat_var <- function(term) {
    tt <- canonical_term(term)

    if (tt %in% cat_vars) return(tt)

    for (v in cat_vars) {
      if (startsWith(tt, v)) return(v)
    }
  ...
  pretty_from_mod <- function(term) {
    tt <- canonical_term(term)
    if (tt %in% names(mod_labels)) return(mod_labels[[tt]])

    v <- detect_cat_var(term)
    if (!is.na(v) && v %in% names(mod_labels)) return(mod_labels[[v]])

    term
  }
  
  pretty_sigma <- function(term) {

    # exact sigma
    if (term == "sigma")
      return("Residual SD")
  
    if (!startsWith(term, "sigma_"))
      return(term)
  
    raw <- sub("^sigma_", "", term)
  
    # direct match
    if (raw %in% names(MOD_LABELS)) {
      return(paste0("Residual SD: ", MOD_LABELS[[raw]]))
    }
  
    # underscore-insensitive match
    raw_clean  <- gsub("_", "", raw)
    keys_clean <- gsub("_", "", names(MOD_LABELS))
  
    idx <- match(raw_clean, keys_clean)
  
    if (!is.na(idx)) {
      return(paste0(
        "Residual SD: ",
        MOD_LABELS[[names(MOD_LABELS)[idx]]]
      ))
    }
  
    paste0("Residual SD: ", raw)
  }

  # Contrast extractor
  extract_contrast_info <- function(term) {

    var <- detect_cat_var(term)
    if (is.na(var)) return("")

    spec <- CAT_SPECS[[var]]
    lvls <- LEVEL_MAP |> dplyr::filter(moderator == var)
    if (nrow(lvls) == 0) return("")

    # reference
    ref_code <- if (!is.null(spec$ref)) spec$ref else lvls$orig[1]

    ref_pretty <- lvls |>
      dplyr::filter(orig == ref_code) |>
      dplyr::pull(pretty)

    ref_pretty <- if (length(ref_pretty)) ref_pretty[1] else ref_code

    tt <- canonical_term(term)

    # CASE 1: binary categorical converted to 0/1
    if (tt == var) {

      if (!is.null(spec$keep) && length(spec$keep) >= 2) {
        other_code <- spec$keep[spec$keep != ref_code][1]
      } else {
        other_code <- lvls$orig[lvls$orig != ref_code][1]
      }

      other_pretty <- lvls |>
        dplyr::filter(orig == other_code) |>
        dplyr::pull(pretty)

      other_pretty <- if (length(other_pretty)) other_pretty[1] else other_code

      return(paste0(ref_pretty, " → ", other_pretty))
    }

    # CASE 2: multi-level categorical (suffix present)
    token <- sub(paste0("^", var), "", tt)

    if (nzchar(token)) {
      lev_pretty <- lvls |>
        dplyr::filter(orig == token) |>
        dplyr::pull(pretty)

      lev_pretty <- if (length(lev_pretty)) lev_pretty[1] else token
      return(paste0(ref_pretty, " → ", lev_pretty))
    }

    ""
  }

  term_is_cat <- function(term) {
    !is.na(detect_cat_var(term))
  }
  ...
  if ("fixed" %in% include) {
    ...
    fx <- fx |>
      ...
      dplyr::mutate(
        Predictor = purrr::map_chr(term, pretty_from_mod),
        Contrast  = purrr::map_chr(term, extract_contrast_info),
        type      = ifelse(vapply(term, term_is_cat, logical(1)), "cat", "cont")
      ) |>
      dplyr::arrange(desc(type))

    parts$fixed <- fx |>
      dplyr::select(Predictor, Contrast,
                    Estimate, Est.Error, Rhat, Bulk_ESS, Tail_ESS)
  }

  # Sigma parameters
  if ("sigma" %in% include && !is.null(s$spec_pars)) {
    ...
    sg <- sg |>
      dplyr::filter(!grepl("Intercept", term, ignore.case = TRUE)) |>
      dplyr::mutate(
        Predictor = purrr::map_chr(term, pretty_sigma),
        Contrast  = ""
      )
  ...
  # Combine + GT
  tab <- dplyr::bind_rows(parts, .id = "component")

  gt::gt(tab, groupname_col = "component") |>
    gt::tab_header(
      title = paste0(
        title,
        if (!is.null(model_name)) paste0(" — ", model_name),
        if (!is.null(trajectory_name)) paste0(" (", trajectory_name, ")")
      ),
      subtitle = subtitle
    ) |>
    gt::fmt_number(
...
loo_gt_table <- function(
  ...
  if (add_weights) {
    ...
    df$Weight <- round(as.numeric(w[match(df$Model, names(w))]), 3)
  }
  ...
  gt_tbl <- df %>%
    ...
    cols_label(
      Model = "Model",
      ELPD_diff = "Δ ELPD (relative to best model)",
      SE = "SE(Δ)",
      Weight = "Model Weight"
    ) %>%
    fmt_number(
    ...
    # Highlight best model: highest ELPD (i.e., Δ = 0)
    tab_style(
      style = list(
...
df_Low_clean <- prepare_data_brms(df_Low, exclude = c("TP1_TPX", "Military"))

# Determine Collinearity
correlations_Low <- generate_correlation_gt(df = df_Low_clean, trajectory = "Low", outcome = "Low_n" )
correlations_Low
...
We compare the multivariate models of the Low trajectory using leave-one-out cross-validation (LOO-CV) to evaluate whether additional moderator domains improve predictive accuracy, ranking models by ELPD (with higher values indicating better performance), interpreting ΔELPD and its SE to gauge relative fit, and using optional model weights to summarize each model’s likelihood of generalizing well to new data.

```{r BRMS-model-comparison-low, results='asis', message=FALSE}
library(brms)
library(purrr)

# Assume 'model_list' is your list of fitted brms objects
# 1. Add LOO to all models in the list
models_Low <- purrr::map(models_Low, function(m) {
  add_criterion(m, "loo", resp = "Lown", moment_match = TRUE)
})

# 2. Compare them using the 'invoke' or 'do.call' method
comparison <- loo_compare(models_Low)

# 3. Print the results
print(comparison, simplify = FALSE)# Compare models using LOO-CV
...
# Prepare the data (cleaning, standardizing, recoding)
df_Decreasing_clean <- prepare_data_brms(df_Decreasing, exclude = c("TP1_TPX", "Military"))

# Determine Collinearity
correlations_Decreasing <- generate_correlation_gt(df = df_Decreasing_clean, trajectory = "Decreasing", outcome = "Decreasing_n")
...
# Prepare the data (cleaning, standardizing, recoding)
df_Increasing_clean <- prepare_data_brms(df_Increasing, exclude = c("TP1_TPX", "Military"))

# Determine Collinearity
correlations_Increasing <- generate_correlation_gt(df = df_Increasing_clean, trajectory = "Increasing", outcome = "Increasing_n")
...
# Prepare the data (cleaning, standardizing, recoding)
df_High_clean <- prepare_data_brms(df_High, exclude = c("TP1_TPX", "Military"))

# Determine Collinearity
correlations_High <- generate_correlation_gt(df = df_High_clean, trajectory = "High", outcome = "High_n")
correlations_High
...
# Prepare the data (cleaning, standardizing, recoding)
df_Moderate_clean <- prepare_data_brms(df_Moderate, exclude = c("TP1_TPX", "Military"))

# Determine Collinearity
correlations_Moderate <- generate_correlation_gt(df = df_Moderate_clean, trajectory = "Moderate", outcome = "Moderate_n")
