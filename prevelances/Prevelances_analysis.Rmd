---
title: "Prevalences analysis"
author: "Coimbra, van de Schoot"
date: "2025-07-20"
output: 
  html_document:
    toc: true
...
```{r load-data}
# load data
df_moderation <- read_csv2("data/data_for_moderation_analyses.csv")
...
```{r resilient-sensitivity_1, message=FALSE, warning=TRUE}
# Ensure integer event counts (xi) and sample sizes (ni) are provided for metafor
# Assumed columns: Resilient_n (events) and Sample_Size (total N)
library(metafor)

df_resilient2 <- subset(df_resilient, !is.na(Resilient_n) & !is.na(Sample_Size))
# Coerce to integers if they came in as numeric
df_resilient2$Resilient_n <- as.integer(round(df_resilient2$Resilient_n))
df_resilient2$Sample_Size <- as.integer(round(df_resilient2$Sample_Size))

# Basic sanity checks
stopifnot(all(df_resilient2$Resilient_n >= 0),
          all(df_resilient2$Sample_Size > 0),
          all(df_resilient2$Resilient_n <= df_resilient2$Sample_Size))

# Use rma.glmm with measure="PLO" (logit-transformed proportions) and continuity correction
res_resilient_glmm <- rma.glmm(measure = "PLO",
                               xi = Resilient_n,
                               ni = Sample_Size,
                               data = df_resilient2,
                               model = "CM.EL")

print(res_resilient_glmm)

# Back-transform pooled logit proportion to proportion for readability
pooled_prop <- transf.ilogit(coef(res_resilient_glmm))
ci_prop     <- transf.ilogit(confint(res_resilient_glmm)$random[,"ci.lb":"ci.ub"])

cat(sprintf("Pooled proportion (logit-backtransformed): %.3f
", pooled_prop))

# Optional: forest plot (on logit scale; add transf to show proportions)
forest(res_resilient_glmm, transf = transf.ilogit, refline = transf.logit(0.5),
       xlab = "Proportion (Resilient)", mlab = "Random-effects (GLMM)")
...
We conducted an influence analysis to identify studies with disproportionate impact on the pooled estimate using Cook's distance from a glmer model.

```{r resilient-influential}

# Fit a GLMM with glmer (binomial model with random study effect)
model_glmer_resilient <- glmer(cbind(Resilient_n, Sample_Size - Resilient_n) ~ 1 + (1 | Study),
                     data = df_resilient,
...
**Note on Number of Studies Included**
The number of studies included in the models may differ between rma.glmm() and glmer( control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))).
This is expected because the two methods handle data slightly differently:
...
glmer( control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5))) is a general GLMM function. It uses binomial counts and handles missingness or zero counts differently. Studies with no variance or missing outcomes may be excluded automatically.
...
model_glmer_recovery <- glmer(cbind(Recovery_n, Sample_Size - Recovery_n) ~ 1 + (1 | Study),
                              data = df_recovery, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
...
model_glmer_worsening <- glmer(cbind(Worsening_n, Sample_Size - Worsening_n) ~ 1 + (1 | Study),
                               data = df_worsening, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
...
model_glmer_chronic <- glmer(cbind(Chronic_n, Sample_Size - Chronic_n) ~ 1 + (1 | Study),
                              data = df_chronic, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
...
model_glmer_partially <- glmer(cbind(Partially_n, Sample_Size - Partially_n) ~ 1 + (1 | Study),
                               data = df_moderate, family = binomial, control = glmerControl(optimizer = "bobyqa", optCtrl = list(maxfun = 2e5)))
